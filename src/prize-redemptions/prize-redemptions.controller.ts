...
export class PrizeRedemptionsController {
  ...
  @ApiOperation({
    summary: 'Listar canjes de premios con paginación y filtro exclusivo',
  })
  @ApiQuery({
    name: 'page',
    required: false,
  ...
  @ApiQuery({
    ...
    type: String,
    description:
      'Filtrar por ID de premio. Si no se envia retrna todos los premios canjeados por el usuario.',
  })
  @ApiResponse({
    status: 200,
    description: 'Listado de canjes de premios retornado correctamente',
  })
  @ApiResponse({
    status: 400,
    description: 'Solo puede enviarse user_id o prize_id, no ambos.',
  })
  ...
  @ApiResponse({
    status: 404,
    description: 'No se encontró el canje de premio',
  })
  @ApiResponse({ status: 500, description: 'Error interno del servidor' })
  async findOne(
  ...
  @ApiResponse({
    status: 201,
    description: 'Canje de premio creado exitosamente',
  })
  @ApiResponse({
    status: 400,
    description: 'Datos inválidos para crear el canje de premio',
  })
  @ApiResponse({ status: 500, description: 'Error al crear el miembro' })
  async create(
    @Body() body: CreatePrizeRedemptionDto,
  ): Promise<PrizeRedemption> {
  ...
  @ApiParam({
    name: 'id',
    description: 'UUID del canje de premio a actualizar',
  })
  @ApiResponse({
    status: 200,
    description: 'Canje de premio actualizado correctamente',
  })
  @ApiResponse({
    status: 404,
    description: 'No se encontró el canje de premio a actualizar',
  })
  @ApiResponse({
    status: 500,
    description: 'Error al actualizar el canje de premio',
  })
  async update(
    @Param('id', ParseUUIDPipe) id: string,
    @Body() body: UpdatePrizeRedemptionDto,
  ) {
  ...
  @ApiResponse({
    status: 204,
    description: 'Canje de premio eliminado correctamente',
  })
  @ApiResponse({
    status: 404,
    description: 'No se encontró el canje de premio a eliminar',
  })
  @ApiResponse({
    status: 409,
    description: 'No se puede eliminar el canje de premio (conflicto)',
  })
