...
@Injectable()
export class CompleteWithdrawUseCase {
  ...
  async execute(dto: WithdrawResponseDto, manager: EntityManager): Promise<UserWithdraw> {
    ...
    // Validar que continúe en estado PENDING
    ...
    // 4 & 5) Descuento Definitivo y actualización de la transacción
    await this.walletPaymentService.consumeLockedFunds(
      manager,
      userWallet.id,
